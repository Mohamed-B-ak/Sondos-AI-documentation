...
{/* - If you opt for the auth route, you need to specify the username and password in the next step when importing the number to Sondos
   <Frame>
     <img
...
- Currently Sondos SIP server does not have a static IP, so if you opt for the IP route, you need to whitelist all the IP addresses in the range like following:
...
- Here you will specify Sondos SIP server address as the origination SIP URI: ```sip:5wed79aeavr.sip.livekit.cloud ``` 
...
Now the number is set up with your elastic SIP trunking, you need to import the number to Sondos so that we will know how to route the call.
...
{/* - If you opted for auth route select Username and password.
   ...
   <Frame>
     <img
       ...
       alt="Twillio Sondos Credentials Auth Type"
     />
   </Frame> */}
...
```
   <Frame>
     <img
       ...
       alt="Twillio Sondos Credentials IP Type"
     />
   </Frame> 
...
7. Select the country of where your SIP trunk is located. 
    ...
    <Frame>
      <img
        ...
        alt="Twillio Sondos SIP Country"
      />
    </Frame>
